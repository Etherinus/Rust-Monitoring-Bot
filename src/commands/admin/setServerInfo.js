...
const { COMMANDS, OPTIONS, DAYS_OF_WEEK, RUST_MODES, RUST_TEAM_LIMITS, RUST_MAPS, EMBED_DEFAULTS } = require('../../utils/constants');
const { getNextWipeTimestamp, getNextDailyTimestamp } = require('../../utils/timeUtils');
const { CommandError } = require('../../errors/AppError');
const config = require('../../config');
const logger = require('../../utils/logger').child({ service: 'Cmd:SetServerInfo' });
...
module.exports = {
    data: addServerInfoOptions(
        new SlashCommandBuilder()
            .setName(COMMANDS.SET_SERVER_INFO)
            .setDescription('Create or update the main server info embed')
    ...
    async execute(interaction, { embedDataService }) {
        ...
        try {
            ...
            embedDataService.setEmbeds([embed]);
            await embedDataService.updateDiscordMessage(interaction.client, interaction.channel);

            logger.info(`Main server info embed updated by ${interaction.user.tag} in guild ${interaction.guildId}.`);
            await interaction.editReply('✅ Main server info embed updated successfully!');

        } catch (error) {
            logger.error(`Error executing /${COMMANDS.SET_SERVER_INFO}: ${error.message}`, { userId: interaction.user.id, guildId: interaction.guildId, stack: error.stack });
